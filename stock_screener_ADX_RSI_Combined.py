...
stock_data = get_historical_data(ticker, '2010-01-01')
...
plot_data = stock_data[stock_data.index >= '2020-01-01']
...
buy_price, sell_price, adx_rsi_signal = adx_rsi_strategy(stock_data['Close'], stock_data['adx'], stock_data['plus_di'], stock_data['minus_di'],
                                                         stock_data['rsi_14'])

plt.plot(stock_data['Close'])
plt.plot(stock_data.index, buy_price, marker='^', markersize=10, color='green')
plt.plot(stock_data.index, sell_price, marker='v', markersize=10, color='r')
...
adx = stock_data['adx']
pdi = stock_data['plus_di']
ndi = stock_data['minus_di']
rsi = stock_data['rsi_14']
Close_price = stock_data['Close']
adx_rsi_signal = pd.DataFrame(adx_rsi_signal).rename(columns={0: 'adx_rsi_signal'}).set_index(stock_data.index)
position = pd.DataFrame(position).rename(columns={0: 'adx_rsi_position'}).set_index(stock_data.index)

frames = [Close_price, adx, pdi, ndi, rsi, adx_rsi_signal, position]
strategy = pd.concat(frames, join='inner', axis=1)

print(strategy.tail())
print(strategy[45:50])
...
for i in range(len(stock_data_ret)):
    returns = stock_data_ret['returns'][i] * strategy['adx_rsi_position'][i]
...
print(cl('Profit gained from the ADX RSI strategy by investing $100k in stock_data : {}'.format(total_investment_ret),
         attrs=['bold']))
...
print(cl('ADX RSI Strategy profit is {}% Higher than the Benchmark Profit'.format(
